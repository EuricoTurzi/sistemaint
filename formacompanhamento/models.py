...
class clientes_acionamento(models.Model):
    ...
    franquiakm = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True)
    franquiahora = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True)
    valordeacionamento = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True)
    ...
    def clean(self):
        if self.franquiakm is not None and self.franquiakm < 0:
            raise ValidationError("A franquia em KM não pode ser negativa.")
        if self.franquiahora is not None and self.franquiahora < 0:
            raise ValidationError("A franquia em horas não pode ser negativa.")
        if self.valordeacionamento is not None and self.valordeacionamento < 0:
            raise ValidationError("O valor de acionamento não pode ser negativo.")
...
class RegistroPagamento(models.Model):
    ...
    tipo1 = [
        ...
        ('Antenista e Pronta Resposta Desarmado', 'Antenista e Pronta Resposta Desarmado'),
        ('Pronta Resposta Armado e Pronta Resposta Desarmado', 'Antenista e Pronta Resposta Desarmado'),
    ]
    contato = [
        ('Whatsapp', 'Whatsapp'),
        ('E-mail', 'E-mail'),
    ...
    tipo = models.CharField(choices=tipo1, max_length=100, null=True,blank=True)
...
class prestadores(models.Model):
    ...
    franquia_hora  = models.DecimalField(max_digits=5, decimal_places=2,null=True,blank=True)
    valorkm  = models.DecimalField(max_digits=5, decimal_places=2,null=True,blank=True)
    valorh  = models.DecimalField(max_digits=5, decimal_places=2,null=True,blank=True)
